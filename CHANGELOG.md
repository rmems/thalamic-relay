...
- Split deterministic safety policy from privileged GPU actuation into a new pure `safety` module: instantaneous `classify()` (Ok/Warn/Critical, fail-closed on missing/invalid/stale/unavailable telemetry), a pure `SafetyStateMachine` for brake/release hysteresis (no NVML, subprocess, global state, or async), and a `SafetyActuator` trait boundary. The NVML/`nvidia-smi` backend (`NvmlActuator` in `gpu`) is now an adapter that implements the trait but defines no safety semantics. Actuator failures are typed and observable (`ActuatorError`) instead of stringly coupled to the supervisor loop. A `FakeActuator` enables deterministic hardware-free tests. `SafetyStatus` moved from `gpu` to `safety` (GH#46)
